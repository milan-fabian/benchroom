...
<h2>
    ...
    for <b><c:out value="${suite.name}"/></b> with <b><c:out value="${run.benchmarkParameter.name}"/></b>
</h2>
                ...
                <td>
                    <c:choose>
                        <c:when test="${run2.id == run.id}">
                            <b>${run2.benchmarkParameter.name}</b>
                        </c:when>
                        <c:otherwise>
                            <a href="<%=request.getContextPath()%><%=URL_BENCHMARK_COMPARE%>?run=${run2.id}">${run2.benchmarkParameter.name}</a>
                        </c:otherwise>
                    </c:choose>
                </td>
